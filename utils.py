...
def advect_RD_with_inputs(model, t, y0, u):
    dt = model['dt']
    N = len(t) - 1
    x = np.full((model['SSMDim'], N+1), np.nan)
    y_pred = np.full((len(y0), N+1), np.nan)
    y_pred[:, 0] = y0
    xdot = np.full((model['SSMDim'], N+1), np.nan)

    for i in range(N):
        try:
            if i == 0:
                x[:, i] = model['V'].T @ (y0 - model['q_bar'])
            xdot[:, i] = (model['r_coeff'] @ phi(x[:, i].reshape(-1, 1), model['ROMOrder'])).flatten() + model['B'] @ (u[:, i] - model['u_bar'])
            # Forward-Euler step
            x[:, i+1] = x[:, i] + dt * xdot[:, i]

            y_pred[:, i+1] = (model['w_coeff'] @ phi(x[:, i+1].reshape(-1, 1), model['ROMOrder'])).T + model['q_bar']
        except Exception as e:
            raise e
    
    return t, x, y_pred, xdot
...
def interpolate_time_series(t, z, u, dt):
    # Create new time array with uniform dt
    new_t = np.arange(np.min(t), np.max(t), dt)

    # Create interpolation functions for z and u
    z_interp = interp1d(t, z, kind='linear', fill_value="extrapolate")
    u_interp = interp1d(t, u, kind='linear', fill_value="extrapolate")

    # Interpolate z and u for new time points
    new_z = z_interp(new_t)
    new_u = u_interp(new_t)

    return new_t, new_z, new_u

def generate_obstacle_constraint(obstacleDiameter, obstacleLoc):
    Hz = np.zeros((2, 3))
    Hz[0, 0] = 1
    Hz[1, 1] = 1
    return CircleObstacle(A=Hz, center=obstacleLoc, diameter=obstacleDiameter)

def get_violations_ratio(task_obstacle, z_traj):
    viol_values = np.array([task_obstacle.get_constraint_violation(x=None, z=z) for z in z_traj])
    
    viol_idxs = [idx for idx, val in enumerate(viol_values) if val]
    viol_ratio = len(viol_idxs) / len(viol_values)
...
def is_dominated(point, points):
    for pt in points:
        if pt[0] <= point[0] and pt[1] <= point[1] and (pt[0] < point[0] or pt[1] < point[1]):
            return True
    return False

def get_violation_values(task_obstacle, z_traj):
    viol_values = np.array([task_obstacle.get_constraint_violation(x=None, z=z) for z in z_traj])

    viol_idxs = [idx for idx, val in enumerate(viol_values) if val]
    viol_ratio = len(viol_idxs) / len(viol_values)

    max_viol = np.max(viol_values)

    return viol_ratio, max_viol

def get_pareto_front(points, x_lim=10., y_lim=10**8):
    pareto_points = []
    for point in points:
    ...
    for i in range(len(pareto_points)):
        if i == 0:
            stair_step_points.append((pareto_points[i][0], y_lim))
        else:
    ...
    if pareto_points:
        stair_step_points.append((x_lim, pareto_points[-1][1]))
...
def get_pareto_data(robot, params, y_metric, dts):
    res_loc = params['result_loc']
    sim_prefix = params['sim_prefix']
    t0 = params['t0']
    z_eq = params['Z_EQ']
    z_interp = params['z_interp']
    task = params["control_task"]

    error_metric = nested_dict()
    x_metric_val = nested_dict()

    for control in ["ssm", "linear", "koopman", "tpwl", "DMD"]:

        for dt in dts:
            dt_num = float(dt)
            dt_string = remove_decimal(dt)
            sim_dir = join(res_loc, dt_string)

            # Open file
            simulation_file = join(sim_dir, f"{sim_prefix[control]}.pkl")
            if exists(simulation_file):
                with open(simulation_file, "rb") as f:
                    sim = pickle.load(f)
            else:
                raise RuntimeError(f"Simulation not found: {sim_dir}")

            # Recenter time
            idx = np.argwhere(sim['t'] > t0)[0][0]
            sim['t'] = sim['t'][idx:] - sim['t'][idx]
            z_centered = sim['z'][idx:, 3:] - z_eq
            if robot == "trunk":
                z_centered[:, 2] *= -1

            # Error hard-coded to be with respect to x-y projection
            if task == "Circle" or task == "star":
                error = (z_centered - z_interp(sim['t']))
            else:
                error = z_centered[:, :2] - z_interp(sim['t'])[:, :2]

            # Calculate error metric value (y-axis)
            error_metric[control][dt_string] = get_metric_value(y_metric, error, ts=sim['t'][:error.shape[0]])

            # Calculate and store solve times
            x_metric_val[control][dt_string] = np.mean(sim['info']['solve_times']) / dt_num

    return error_metric, x_metric_val

def plot_bar_chart(ax, params, data, rmse_threshold=280., set_threshold=False):
    # Assuming data is a dictionary of dictionaries in the format: data[control]
    # params includes: robot type, robot display name, color

    CONTROLS = ["ssm", "linear", "koopman", "DMD", "tpwl"]
    robot_type = params['robot']
    legend_name = params['label']
    colors = params['color']
    width = 0.89
    ...
    dt_positions = [i + width for i in range(len(CONTROLS))]
        
    rmse_vals = [data[control] for control in CONTROLS]
    dt_colors = [matplotlib.colors.to_rgb(colors[control]) 
                 for val, control in zip(rmse_vals, CONTROLS)]
    
    hatches = ['' for val in rmse_vals]
    edgecolors = [dt_colors[i] for i, hatch in enumerate(hatches)]
    positions = dt_positions

    # Plot the metric bars
    bars = ax.bar(positions, rmse_vals, width, color=dt_colors, 
                  edgecolor=edgecolors, zorder=2)

    for bar, hatch in zip(bars, hatches):
        bar.set_hatch(hatch)
    
    control_ticks = [0.92, 1.92, 2.92, 3.92, 4.92]
    ...
    for bar, rmse_val in zip(bars, rmse_vals):
        ...
        if np.isclose(round(rmse_val), 0.):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.1,
                    'Baseline', ha='center', va='bottom', fontsize=10, weight='bold')
        
        # Add an arrow pointing up above rmse that exceed threshold
        elif (rmse_val > threshold_val) and set_threshold:
            ax.text(bar.get_x() + bar.get_width() / 2, 0.85*threshold_val, 
                f'↑{round(rmse_val):,}%', ha='center', va='bottom', fontsize=10, 
                bbox=dict(facecolor='white', edgecolor='none', pad=1.0, alpha=0.7), 
                weight='bold')
            
            if robot_type == "diamond":
                arrow_length = 55.0
            else:
                arrow_length = 150.0
            
            arrow = patches.FancyArrow(bar.get_x() + bar.get_width() / 2, 
                                       ax.get_ylim()[1], 0, arrow_length, 
                                       width=0.5*bar.get_width(), 
                                       head_width=0.8*bar.get_width(), 
                                       head_length=arrow_length / 1.8, 
                                       length_includes_head=True, 
                                       color=bar.get_facecolor(), zorder=3)
            ...
            dot_positions = [dots_location, dots_location + 0.5*increment, 
                             dots_location + 1.0*increment]
            
            for dot_y in dot_positions:
                ax.text(bar.get_x() + bar.get_width() / 2, dot_y, '.', 
                        ha='center', va='center', fontsize=20, color='white')
            
        elif rmse_val > 2 * second_max_rmse:
            ax.text(bar.get_x() + bar.get_width() / 2, 0.85*rmse_threshold, 
                    f'↑{round(rmse_val)}%', ha='center', va='bottom', 
